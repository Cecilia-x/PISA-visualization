...
function draw(data){
    ...
    svg.append('line')
       ...
       .attr('x1', margin+30).attr('y1', score_scale(mean_sc))
       .attr('x2', width-margin).attr('y2', score_scale(mean_sc))
       .attr('stroke', 'black');
    var h_average_line_text = svg.append("text").text("World")
                      .attr('y',score_scale(mean_sc)).attr('x', margin).attr('class','axis-title')
                      .attr('fill', 'grey');
    h_average_line_text.append('tspan').text("Average Score").attr('x',margin).attr('dy', '1em');
    svg.append('line')
       .attr('class','vabline')
       .attr('stroke-dasharray','5')
       .attr('y1', margin).attr('x1', escs_scale(mean_es))
       .attr('y2', height-margin-30).attr('x2', escs_scale(mean_es))
       .attr('stroke', 'black');
    var v_average_line_text = svg.append('text').text("World")
                      .attr("x", escs_scale(mean_es)).attr('y', height-margin-20).attr('class', 'axis-title')
                      .attr('fill', 'grey');
    v_average_line_text.append('tspan').text('Average ESCS').attr('dx', '-6em').attr('dy', '1em');

    /*Explanation for Areas*/
    svg.append("text").text("Disadvantage background, low score")
                      .attr('x',margin+10).attr('y', height-margin-10).attr('class','axis-title');
    svg.append("text").text("Advantage background, lower score")
                      .attr('x',margin+escs_scale(-0.2)).attr('y', height-margin-10).attr('class','axis-title');
    svg.append("text").text("Desadvantage background, high score")
                      .attr('x',margin+10).attr('y', margin-10).attr('class','axis-title');
    svg.append("text").text("Advantage background, high score")
                      .attr('x',margin+escs_scale(-0.2)).attr('y', margin-10).attr('class','axis-title');
    ...
    svg.selectAll('circle')
        ...
        .attr('cx', function(d){ return escs_scale(d.MEANESCS);})
        .attr('cy', function(d){ return score_scale(d.MEANSCORE);})
    ...
    /*Add Legends*/
    addLegend($("#graph-area>svg"), margin, 400);
